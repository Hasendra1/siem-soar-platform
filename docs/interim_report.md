...
Preliminary results are reported from test-run executions using the synthetic dataset. These are not final validated results; formal evaluation across multiple repeated trials with live Docker traffic captures will be reported in the final submission.

### 5.1 Model Training Results

The two-tier pipeline, trained on the 270-row synthetic dataset aggregated to 9 device profiles, produced the ensemble scores shown in Table 4.

**Table 4: Per-Device Ensemble Anomaly Scores (Preliminary)**

| Device | IP | Zone | IF Score | DBSCAN Score | Tier 1 Decision | Ensemble Score |
|---|---|---|---|---|---|---|
| **Engineering-WS** | **192.168.10.50** | **OT** | **1.000** | **1.000** | **FLAGGED** | **1.000** |
| CCTV-Camera | 192.168.30.10 | DMZ | 0.158 | 0.000 | PASSED | 0.079 |
| PLC2 | 192.168.10.11 | OT | 0.155 | 0.000 | PASSED | 0.077 |
| Cloud-Gateway | 192.168.30.100 | DMZ | 0.124 | 1.000 | PASSED | 0.062 |
| MQTT-Broker | 192.168.20.100 | IoT | 0.115 | 0.000 | PASSED | 0.057 |
| HMI | 192.168.10.20 | OT | 0.109 | 0.000 | PASSED | 0.055 |
| Sensor-Temp | 192.168.20.10 | IoT | 0.097 | 0.000 | PASSED | 0.048 |
| PLC1 | 192.168.10.10 | OT | 0.080 | 0.000 | PASSED | 0.040 |
| Sensor-Press | 192.168.20.11 | IoT | 0.000 | 0.000 | PASSED | 0.000 |

The attacker device (Engineering-WS) received a maximum ensemble score of 1.0 with both Tier 1 and Tier 2 independently scoring it at 1.0. Tier 1 (Isolation Forest) correctly flagged it as the sole anomalous device, and Tier 2 (DBSCAN) confirmed it as a density outlier. The identification confidence, computed from the score gap between the top-ranked device (1.0) and the runner-up (0.079), was 99.9%.

The score gap of 0.921 between the attacker and the highest-scoring benign device (CCTV-Camera at 0.079) demonstrates clear separation. All eight benign devices scored below 0.08, well beneath the 0.70 alert threshold and far below the 0.90 isolation threshold. No false positives were produced.

The Cloud-Gateway received a DBSCAN score of 1.0 (density outlier) but was classified as PASSED by Tier 1, resulting in a dampened ensemble score of 0.062 — correctly below any action threshold. This demonstrates the value of the two-tier architecture: Tier 1's PASSED classification prevented DBSCAN's outlier detection from triggering a false positive on a device whose traffic pattern (MQTT wildcard subscriber) is genuinely unusual but not malicious.

### 5.2 Isolation and Response

During dynamic pipeline test runs (`run_dynamic_pipeline.py`), the ML monitor successfully detected the attacker's escalating behaviour through the five attack phases and triggered isolation when ensemble scores exceeded 0.90. The segmentation engine disconnected Engineering-WS from both `ot-network` and `dmz-network`, and subsequently isolated PLC1 and PLC2 as COMPROMISED after detecting MALICIOUS_WRITE events targeting their IP addresses. Post-isolation verification confirmed the attacker container was absent from all network container lists.

[INSERT MEASURED ISOLATION LATENCY FROM LIVE RUN — expected to be under 500ms based on Docker SDK disconnect timing observed during development]

### 5.3 Limitations of Preliminary Results

These results were obtained from synthetic data generated by `generate_synthetic_dataset.py`, which simulates the attacker's escalating behaviour with deterministic patterns. While the synthetic data schema matches the real capture pipeline's output exactly, the feature distributions may not fully represent the stochastic variation of real Docker network traffic. Final model accuracy will be reported in the final submission once the 5-minute live capture dataset has been validated across multiple attack scenario repetitions.

[CONFIRM EXACT SILHOUETTE SCORE FROM train_models.py OUTPUT IF COMPUTED IN LATER RUNS]

---

## 6. Work Remaining

The following work items remain to bring the project to completion before the 9 September 2026 deadline.

**Full real-time ML integration**: The real-time inference pipeline currently functions end-to-end in test scenarios, but requires validation under sustained load with concurrent normal and attack traffic running on live Docker containers. The feature extraction bridge between the rolling 30-second window and the trained models must be stress-tested to ensure consistent scoring under varying traffic volumes.

**Wazuh active-response integration**: While Wazuh agents are deployed in all nine device containers and communicate with the Wazuh manager, the active-response module — which would allow Wazuh alerts to directly trigger the ML-based segmentation engine — has not been fully tested. This integration would close the loop between host-based intrusion detection (Wazuh) and network-based anomaly detection (the ML pipeline).

**End-to-end attack scenario validation**: The system must be evaluated across multiple independent attack scenario repetitions (minimum 5 trials per scenario) to compute meaningful precision, recall, and F1 scores. Each trial should use the live Docker traffic capture pipeline (`capture_traffic.py`) running concurrently with the attack simulator (`live_attack_simulator.py`), followed by full pipeline execution through detection and isolation.

**Dashboard refinements**: Minor UI refinements, including the network topology live-update animation during isolation events and historical trend charting on the Dashboard page, remain to be completed.

**Final report writing**: Comprehensive documentation of evaluation methodology, final results, and discussion of findings.

**Table 5: Remaining Work Timeline**

| Week | Dates | Task |
|---|---|---|
| 1–2 | 30 Jun – 13 Jul 2026 | Complete real-time ML integration; stress-test live inference |
| 3–4 | 14 Jul – 27 Jul 2026 | Wazuh active-response integration and testing |
| 5–6 | 28 Jul – 10 Aug 2026 | End-to-end evaluation: 5 repeated attack trials, collect metrics |
| 7–8 | 11 Aug – 24 Aug 2026 | Dashboard polish; additional attack scenarios (if time permits) |
| 9–10 | 25 Aug – 7 Sep 2026 | Final report writing, results analysis, submission preparation |
| — | 9 Sep 2026 | **Final demo deadline** |
